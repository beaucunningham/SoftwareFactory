...
5. Record the result with `npm start -- set-status ui-checked <id>` if the main path works, or `npm start -- set-status ui-failed <id>` if a person cannot finish it. Do not hand-edit `job.json`.
